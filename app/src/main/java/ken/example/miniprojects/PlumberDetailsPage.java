...
public class PlumberDetailsPage extends AppCompatActivity {
RecyclerView plumberRv;
...
FirebaseFirestore firebaseFirestore;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_plumber_details_page);

        getSupportActionBar().setTitle("Plumber");

        plumberRv = findViewById(R.id.plumberRecyclerview);
        firebaseFirestore = FirebaseFirestore.getInstance();

        plumberRv.setHasFixedSize(true);
        plumberRv.setLayoutManager(new LinearLayoutManager(this));
        allServiceAdapter = new AllServiceAdapter(workerdetail,this);
        plumberRv.setAdapter(allServiceAdapter);

        final LayoutAnimationController controller= AnimationUtils.loadLayoutAnimation(this,R.anim.layout_animation_up_to_down);
        plumberRv.setLayoutAnimation(controller);
        plumberRv.scheduleLayoutAnimation();
        ...
        Task<QuerySnapshot> worker =firebaseFirestore.collection("Worker").get().addOnSuccessListener(queryDocumentSnapshots -> {
            ...
            for (DocumentSnapshot d:list){
                if (d.getString("isPlumber") != null && d.contains("emailVerified")){
